...
export default class ProductShowcase extends Component {
  ...
  constructor() {
    ...
    //this._cafeClick = this._cafeClick.bind(this);
  }
  ...
  render() {
    ...
    return (
        ...
        {/* <ViroBox position={[0, -.5, -1]} scale={[.3, .3, .1]} materials={["grid"]} animation={{name: "rotate", run: true, loop: true}} onClick={this._cafeClick}/> */}
        <ViroFlexView style={styles.titleContainer} position={[-7.8, 1, -4]} rotation={[0, 40, 0]} height={4} width={4}
            animation={{ name : this.state.mainAnimation, run : this.state.runAnimation, loop : false }} onClick={this._cafeClick}>
            <ViroText style={styles.prodTitleText} text="Global Kitchen" width={4} height={.5}/>
            <ViroText style={styles.prodTitleText} text="A Kitchen" width={4} height={.5} />
            <ViroText style={styles.prodTitleText} text="B Kitchen" width={4} height={.5} />
            <ViroText style={styles.prodTitleText} text="C Kitchen" width={4} height={.5} />
            <ViroText style={styles.prodTitleText} text="D Kitchen" width={4} height={.5} />
            <ViroNode width={4} height={.5}>
        
        <ViroText text={products[0].productTitleText} width={2} height={.7} style={styles.navButtonText}
 ...
 /*
  ...
  _getVideoIfAvailable() {
    console.log("inside video if available model");
    var product = products[this.state.productToShow];
   ...
   */
  _getNavButtons() {
    console.log("inside get nav model");
    var toReturn = [];
  ...
  _getSingleNavButton(forward, index, centered) {
    console.log("inside get inglenav model");
...
var products = [
    {
      "productTitleText" : "NOODLE",
      "productDescriptionText" : "15255 NE 40th Street | Commons| REDMOND| WA ",
      "productDetails1" : "Tom Yum Noodles",
      "productDetails2" : "Served with a choice of minced pork, shredded chicken, or chinese BBQ pork. Thai noodle with bold tast of fish sauce, lime juice, crushed peanut, and chili flakes. Choice of rice or egg noodles.",
      "productDetails3" : "Price Varies",
      "price" : "$7.99",
      "inStock" : true,
      "online" : true,
      "giftWrappable" : true,
